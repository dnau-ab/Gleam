...
void main() {
	...
	vec3 result = diffuse * 0.25 + lighting + (directional + point);

	const float NUM_SHADES = 3.0f;
	float tempColor = (result.r + result.g + result.b) / 3.0f;
	tempColor = (floor(tempColor * NUM_SHADES)) / NUM_SHADES;
	
	vec3 newResult = vec3(0, 1.5 * tempColor, 0);
	
	fragColor = vec4(newResult, 1.0);
} 