...
void main() {
	...
	int tmp = int(texCoord.x * texSize.x) % sampleRadius;
	if (tmp > sampleRadius / 2) {
		tmp *= -1;
	}
	newTexCoord.x = (texCoord.x * texSize.x + tmp) / texSize.x;
	tmp = int(texCoord.y * texSize.y) % sampleRadius;
	if (tmp > sampleRadius / 2) {
		tmp *= -1;
	}
	newTexCoord.y = (texCoord.y * texSize.y + tmp) / texSize.y;

	vec3 fragPos = texture(gPosition, newTexCoord).rgb;
	vec3 normal = texture(gNormal, newTexCoord).rgb;
	vec3 diffuse = texture(gColorSpec, newTexCoord).rgb;
	float specular = texture(gColorSpec, newTexCoord).a;
	...
	fragColor = vec4(diffuse * 0.1 + result, 1.0);
} 